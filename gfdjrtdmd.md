build a simple tkinter app that allows users to type a message and
then show the user the message backwards
ChatGPT said:
Perfect little starter GUI! 🐍
Here’s a super simple Tkinter app that lets users type a message and then see it backwards
when they click a button.
This version is kid-friendly, clean, and includes lots of comments so middle-schoolers can
follow along easily.
